...
class MainNavigationOverlay(QtWidgets.QFrame):
    ...
    def __init__(
        ...
        *,
        ...
        notice = QtWidgets.QLabel("Changes apply immediately")
        notice.setObjectName("mainNavigationHeaderNotice")
    ...
    def _page(
        ...
        if entry == "Bridge Status":
            ...
            copy_diagnostics.setObjectName("bridgeDiagnosticsButton")
    ...
    def _reset_pending_settings_reset(self) -> None:
        button = getattr(self, "reset_all_settings", None)
        if button is None:
            return
    ...
    def _reset_all_settings_clicked(self) -> None:
        button = self.reset_all_settings
        if button.property("confirmReset") is not True:
            button.setText("Confirm reset")
            button.setToolTip("Click again to reset all settings to defaults")
            button.setProperty("confirmReset", True)
            button.style().unpolish(button)
            button.style().polish(button)
            return

        self._reset_pending_settings_reset()
        self.settings_panel.reset_all_to_defaults()
        notify(self, "Settings reset to defaults")
