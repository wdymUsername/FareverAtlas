...
from .config import LIVE_NAME, POI_NAME, PROJECT_ROOT
...
class DataHub(QtCore.QObject):
    ...
    def __init__(self, state_dir: Path):
        ...
        self.live_file = state_dir / LIVE_NAME
        self.native_live_file = PROJECT_ROOT / "native_bridge/farever-telemetry.json"
        self.poi_file = state_dir / POI_NAME
        self.asset_poi_file = PROJECT_ROOT / "assets/pois_W1_Siagarta.json"
        self.state: dict[str, Any] = {}
        self.pois: list[dict[str, Any]] = []
        self._live_mtime_ns = -1
        self._poi_mtime_ns = -1
        self._last_good_monotonic: float | None = None
        self._active_live_file = self.live_file
    ...
    def _load_cached_pois(self) -> None:
        for path in (self.asset_poi_file, self.poi_file):
            try:
    ...
    def _emit_offline(self) -> None:
        self.updated.emit(
            Snapshot(
                {}, self.pois, False, "Offline", None,
                str(self.live_file), str(self._active_poi_file),
            )
        )
    ...
    def _select_live_file(self) -> Path:
        try:
            if time.time() - self.native_live_file.stat().st_mtime < 2.0:
                return self.native_live_file
        except OSError:
            pass
    ...
    def poll(self) -> None:
        ...
        try:
            active_live_file = self._select_live_file()
            if active_live_file != self._active_live_file:
                self._active_live_file = active_live_file
                self._live_mtime_ns = -1
            stat = active_live_file.stat()
            if stat.st_mtime_ns != self._live_mtime_ns:
                payload = self._read_json(active_live_file)
                if not isinstance(payload, dict) or payload.get("schema") != 1:
                    raise ValueError("unsupported live-state schema")
                self.state = (
                    self._normalize_native(payload)
                    if active_live_file == self.native_live_file
                    else payload
                )
            ...
            native_connected = (
                active_live_file != self.native_live_file
                or self.state.get("native_bridge", {}).get("state") == "connected"
            )
        ...
        try:
            active_poi_file = (
                self.asset_poi_file if self.asset_poi_file.is_file() else self.poi_file
            )
            if active_poi_file != self._active_poi_file:
                self._active_poi_file = active_poi_file
                self._poi_mtime_ns = -1
            stat = active_poi_file.stat()
            if stat.st_mtime_ns != self._poi_mtime_ns:
                payload = self._read_json(active_poi_file)
                raw = payload.get("pois", []) if isinstance(payload, dict) else payload
                if isinstance(raw, list):
                    self.pois = [item for item in raw if isinstance(item, dict)]
