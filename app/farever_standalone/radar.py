...
class RadarWidget(QtWidgets.QWidget):
    ...
    def set_offline_mode(
        ...
        """Keep a valid map center offline without retaining a live player marker."""
        if not offline:
    ...
    def paintEvent(self, event: QtGui.QPaintEvent) -> None:  # noqa: N802
        ...
        if self.show_custom_waypoints:
            ...
            if active_waypoint is not None and math.isfinite(player_x) and math.isfinite(player_y):
        ...
        for member in party if self.show_party_members and isinstance(party, list) else []:
            ...
            if (
                member.get("attributes_valid", True)
        ...
        if math.isfinite(player_x) and math.isfinite(player_y):
            ...
            # to the standalone triangle's forward axis. Correct the marker
